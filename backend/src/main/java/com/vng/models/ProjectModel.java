...
import com.vng.dal.entities.ProjectDurationChangelog;
import com.vng.dal.entities.ProjectFaseChangelog;
import com.vng.dal.entities.ProjectNameChangelog;
import com.vng.dal.entities.ProjectPlanTypeChangelog;
import com.vng.dal.entities.ProjectPlanologischePlanstatusChangelog;
import com.vng.dal.entities.ProjectPrioriseringChangelog;
import com.vng.dal.entities.ProjectState;
    ...
    public ProjectModel(Project project) {
        ...
        for (ProjectPrioriseringChangelog item : project.getPriority()) {
            ...
            if (item.getValue() != null) {
                data.setData(item.getMinValue().getState().get(0).getValueLabel());
                data.setWeight(item.getMinValue().getState().get(0).getOrdinalLevel());                
            }
            data.setStartDate(item.getStartMilestone());
            data.setEndDate(item.getEndMilestone());
            priority.add(data);
