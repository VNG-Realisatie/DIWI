        ...
        CASE
            ...
            WHEN pgv.physical_appearance_options IS NOT NULL THEN
                (
                    WITH physical_appearance_values AS
                        (
                            SELECT unnest(hv.physical_appearance) AS pa_val
                            FROM
                                houseblocks_view hv
                            WHERE hv.delivery_date >= pgv.start_date AND hv.delivery_date <= pgv.end_date
                        )
                    SELECT COALESCE ( SUM(cast (pav.pa_val->>'amount' AS integer)), 0)
            ...
            WHEN pgv.house_type_options IS NOT NULL THEN
                (
                    WITH housetype_values AS
                        (
                            SELECT unnest(hv.house_type) AS ht_val
                            FROM
                                houseblocks_view hv
                            WHERE hv.delivery_date >= pgv.start_date AND hv.delivery_date <= pgv.end_date
                        )
                    SELECT COALESCE ( SUM(cast (htv.ht_val->>'amount' AS integer)), 0)
            ...
            WHEN pgv.target_group_options IS NOT NULL THEN
                (
                    WITH targetgroup_values AS
                        (
                            SELECT unnest(hv.target_group) AS tg_val
                            FROM
                                houseblocks_view hv
                            WHERE hv.delivery_date >= pgv.start_date AND hv.delivery_date <= pgv.end_date
                        )
                    SELECT COALESCE ( SUM(cast (tgv.tg_val->>'amount' AS integer)), 0)
            ...
            WHEN pgv.ground_position_options IS NOT NULL THEN
                (
                    WITH groundposition_values AS
                        (
                            SELECT unnest(hv.ground_position) AS ht_val
                            FROM
                                houseblocks_view hv
                            WHERE hv.delivery_date >= pgv.start_date AND hv.delivery_date <= pgv.end_date
                        )
                    SELECT COALESCE ( SUM(cast (gpv.ht_val->>'amount' AS integer)), 0)
            ...
            WHEN pgv.ownership_value_options IS NOT NULL THEN
                (
                    ...
                    SELECT
                        COALESCE(SUM(cast (hov.amount AS integer)), 0)
                    FROM
