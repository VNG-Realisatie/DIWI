...
for geo_gemeente_path in all_gemeente_geojson_paths:
    ...
    df_in = df_in[df_in['geometry.type'].notna()]
    df_out = func.create_df_outs(df=df_in)

    df_out = func.fill_type_geometry(df_out=df_out, df_in=df_in)
    df_out = func.fill_projectgegevens(df_out=df_out, df_in=df_in)
    ...
    func.write_to_geojson(json_out=json_out, gemeente=gemeente_name)