...
export const StartDatePicker = ({ houseBlockStartDate, updateHouseBlockStartDate, edit, editForm }: Props) => {
    return (
        <Stack width="100%">
            ...
            {!edit && editForm && (
                <InputContainer>
                    <Typography>{houseBlockStartDate}</Typography>
                </InputContainer>
            )}
            {!edit && !editForm && (
                <DatePicker format="DD/MM/YYYY" value={houseBlockStartDate ? dayjs(houseBlockStartDate) : null} onChange={updateHouseBlockStartDate} />
            )}
        </Stack>
    );
};
